...
resource "aws_glue_catalog_table" "this" {
  ...
  name               = var.catalog_table_name != null ? var.catalog_table_name : "${var.name_prefix != null ? var.name_prefix : "glue-table"}"
...
resource "aws_lakeformation_permissions" "default" {
  count = var.lakeformation_permissions_enabled && local.enabled ? 1 : 0
  ...
  table {
    database_name = var.database_name
    name          = aws_glue_catalog_table.this[0].name
  }

  depends_on = [aws_glue_catalog_table.this]
}
